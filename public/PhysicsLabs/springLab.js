...
function Slinky(parts, mass, k, topY, aY) {
    ...
    this.topY = topY;
    this.parts = [];
    var length = mass*aY/(2*k); // why on earth is this the length?
    console.log(length);
    var y = 2./(parts*parts-parts); // reciprocal of (sum from 1 to parts-1)
    var currentY = topY;

    for (var part=0; part<parts; part++) {
        // start at equilibrium by making ratios of 1, 2, 3, etc. from the bottom up
        // this can be solved for with three matrix equations, one for the geometry of extension, one for how extensions create forces, and one for force equilibrium.
